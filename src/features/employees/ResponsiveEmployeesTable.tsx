...
const ResponsiveEmployeesTable = ({
	...
	const selectEmployee = (projectId: string) => {
		selectedEmployee === projectId ? setSelectedEmployee('') : setSelectedEmployee(projectId);
	};

	return (
		<div className='w-full rounded-md border border-ashen-grey bg-white'>
			<TableHeader label='Projects Table' total={totalNumberOfEmployees} value={value} handleSearch={handleSearch} />
