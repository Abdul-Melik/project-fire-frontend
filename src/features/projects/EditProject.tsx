...
const EditProject = ({ project, closeEditProjectSideDrawer }: Props) => {
  ...
  useEffect(() => {
    if (project) {
      ...
      setActualEndDate(project.actualEndDate ? new Date(project.actualEndDate) : null);
      ...
      <Main className='rounded-lg bg-white p-6 '>
        <form className='flex flex-col gap-4'>
          ...
          <ProjectTypeSelector
            ...
            handleProjectTypeSelection={(projectType) => setSelectedProjectType(projectType)}
          />
          <SalesChannelSelector
            selectedSalesChannel={selectedSalesChannel}
            handleSalesChannelSelection={(salesChannel) => setSelectedSalesChannel(salesChannel)}
          />
          <DateInputs
            label='Duration'
          ...
          {selectedProjectStatus === "Completed" && (
            <DateInput
              ...
              className='w-full'
              handleDateSelection={(actualEndDate) => setActualEndDate(actualEndDate)}
            />
          )}
          <EmployeesSelector
          ...
          <InputField
            ...
            handleInput={(projectVelocity) => setProjectVelocity(projectVelocity)}
          />
        </form>
      </Main>
      <Footer
