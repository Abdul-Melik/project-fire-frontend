...
const Avatar = ({ src, alt, className }: AvatarProps) => {
	...
	return (
		<div className='relative hover:cursor-pointer' onMouseOver={handleHover} onMouseLeave={handleLeave}>
			<img
...
const Avatars = ({ images }: Props) => {
	...
	const avatars = images.map((image, index) => ({
		src: image,
		alt: `avatar-${index}`,
	}));
	...
	return (
		<div className='flex flex-wrap -space-x-3'>
			{avatars.slice(0, maxVisibleAvatars).map((avatar, index) => (
				<Avatar key={index} src={avatar.src} alt={avatar.alt} className='' />
			))}
			{overflowCount > 0 && (
				<div className=''>
