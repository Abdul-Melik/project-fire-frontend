...
const SearchBar = ({ placeholder, value, onChange, onClick, handleSearch }: Props) => {
	return (
		<form
			className='relative'
			onSubmit={event => {
		...
		>
			<input
				...
				onChange={onChange}
			/>
			<img className='absolute left-3 top-2 h-6 w-6 cursor-pointer' src={search} alt='Search icon' />
			{value && <img className='absolute right-3 top-3 h-4 w-4' src={close} alt='Clear icon' onClick={onClick} />}
		</form>
	);
};
