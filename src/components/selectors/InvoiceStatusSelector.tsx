...
const InvoiceStatusSelector = ({ selectedInvoiceStatus, handleInvoiceStatusSelection }: Props) => {
	const ref = useRef<HTMLDivElement | null>(null);
	const [width, setWidth] = useState(0);
	const [height, setHeight] = useState(0);
	const [isInvoiceStatusSelectorOpen, setIsInvoiceStatusSelectorOpen] = useState(false);
		...
		<div className='flex flex-col gap-1'>
			...
			<div ref={ref} className='relative rounded-md border border-misty-moonstone px-4 py-2 focus:outline-none'>
				<div
					...
					onClick={() => setIsInvoiceStatusSelectorOpen(!isInvoiceStatusSelectorOpen)}
				>
					<span className='font-gilroy-regular text-sm font-normal leading-[22px] text-slate-mist'>
						{selectedInvoiceStatus
							? getInvoiceColorAndStatus(selectedInvoiceStatus as InvoiceStatus)?.status
							: 'Select project status'}
					</span>
					<img
						className={`transition ${isInvoiceStatusSelectorOpen ? 'rotate-180' : ''}`}
						src={chevronDown}
						alt='Down icon'
					/>
				</div>
					...
					>
						<RadioButton
							...
							handleRadioButtonChange={event => {
								handleInvoiceStatusSelection(event.target.checked ? 'Sent' : '');
						...
						<RadioButton
							...
							handleRadioButtonChange={event => {
								handleInvoiceStatusSelection(event.target.checked ? 'Paid' : '');
								setIsInvoiceStatusSelectorOpen(false);
							}}
						/>
					</div>
