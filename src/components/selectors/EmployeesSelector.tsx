...
const EmployeesSelector = ({ selectedEmployees, handleEmployeesSelection }: Props) => {
		...
		if (data) {
			...
			setOpenPartTimeSelectors(
				openPartTimeSelectors.filter(employeeId =>
					data.employees.some((employee: Employee) => employee.id === employeeId)
				)
			);
		}
	}, [data]);
		...
		<div className='flex flex-col gap-1'>
			...
			<div ref={ref} className='relative rounded-md border border-misty-moonstone px-4 py-2 focus:outline-none'>
				...
				>
					...
					<img
						className={`transition ${isEmployeesSelectorOpen ? 'rotate-180' : ''}`}
						src={chevronDown}
						alt='Down icon'
					/>
				</div>
						...
						>
							...
							<div className='flex items-center gap-[10px]'>
								<div className='relative min-w-[90px] rounded-md border border-misty-moonstone px-2 py-1 focus:outline-none'>
									...
									>
										...
										<span className='font-gilroy-regular text-xs font-normal text-evergreen'>
											{!openPartTimeSelectors.includes(employee.id) ? (!partTime ? 'Full time' : 'Part time') : ''}
										</span>
										<img
											className={`transition ${
												openPartTimeSelectors.find(employeeId => employeeId === employee.id) ? 'rotate-180' : ''
											}`}
											src={chevronDown}
											alt='Down icon'
										/>
								...
								<img
									...
									onClick={() => {
										handleEmployeesSelection(
											selectedEmployees.filter(({ employee: selectedEmployee }) => selectedEmployee.id !== employee.id)
										);
										setOpenPartTimeSelectors(openPartTimeSelectors.filter(employeeId => employeeId !== employee.id));
									}}
								/>
							</div>
						</div>
